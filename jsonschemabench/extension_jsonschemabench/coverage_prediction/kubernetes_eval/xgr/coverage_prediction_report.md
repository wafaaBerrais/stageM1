...
- OVER rows: 1680 ({'CORRECT_VALID': 1334, 'OVER': 346})
...
- For `xgr`, both `UNDER` and `OVER` modeling tables exclude `compile_error` rows and keep only runtime `passed`/`failed` results.
- For `xgr`, the `UNDER` model uses all usable refined runtime features from the fused feature table.
- For `xgr`, the `OVER` model uses an expanded runtime feature set covering schema size, advanced JSON Schema keywords, object/string/array/enum violations, type mismatches, and instance shape.

## Retained artifacts

- Shared feature CSV: [Kubernetes_external_features.csv](../../external_eval/Kubernetes/xgr/Kubernetes_external_features.csv). The identical local feature CSV copies were removed.
